  ...
  PreProcess: function (element) {
	...
	MathMLMathSpeak: {
		...
		getMathSpeak: function(e, d) {
			...
			var depends = this.dependsOnPreviousElement(item, hex);
			console.log("DependsOnPreviousElement: " + depends);
			if (depends) {return this.dependsMathSpeak(item, hex);}
			if (e === "mn") { //math number
      ...
      return false;
		...
		dependsOnPreviousElement: function(item, hex) {
			...
			if (hex === "2212") return true; //2212 is minus sign
		...
		dependsMathSpeak: function(item, hex) {
			...
			if (hex === "2212") {  // if minus
				if (this.previous === "") return "Negative ";
				return "Minus ";
				//if previous is empty return negative, else return minus
			}
			if (item === "(") {
				if (this.previous !== "") {
					var item = this.previous.data[0].data[0];
					var hex = item.charCodeAt().toString(16).toUpperCase();
					if (item === "f" || item === "g" || this.trig[item]) {
